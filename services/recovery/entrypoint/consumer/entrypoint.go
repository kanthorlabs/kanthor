...
func New(
	...
	logger = logger.With("service", "recovery", "entrypoint", "consumer")
	return &consumer{
		conf:       conf,
		logger:     logger,
		...
		healthcheck: background.NewServer(
			healthcheck.DefaultConfig("recovery.consumer"),
...
func (service *consumer) Run(ctx context.Context) error {
	tracectx := context.WithValue(ctx, telemetry.CtxTracer, telemetry.Tracer(project.Name("recovery_consumer")))
